...
INTEGRATION_SRC="$SCRIPT_DIR/bash-integration.sh"
...
if [ $# -ge 1 ]; then
    ...
    echo ""
    echo "  You can also edit $HOME/.hermes/scripts/proxy-forwarder.py"
    echo "  or create ~/.hermes/scripts/proxy-config.json to set it later."
    REMOTE_HOST=""
    REMOTE_PORT="443"
fi
# Save config
...
if grep -q "proxy-forwarder.py" "$HOME/.bashrc" 2>/dev/null; then
...
git config --global http.proxy http://127.0.0.1:10808 2>/dev/null && echo "  ✓ git proxy set"
npm config set proxy http://127.0.0.1:10808 2>/dev/null && npm config set https-proxy http://127.0.0.1:10808 2>/dev/null && echo "  ✓ npm proxy set"
...
if command -v agent-browser &>/dev/null; then
    ...
    npm install -g agent-browser 2>/dev/null && agent-browser install 2>/dev/null && echo "  ✓ agent-browser installed"
fi
